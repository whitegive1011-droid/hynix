...
""",
    ...
    assert workbook["Dashboard"]["B4"].value == "Manual Upload Only"
...
def test_main_uses_csv_even_when_config_mentions_external_provider(
    tmp_path: Path,
    monkeypatch,
) -> None:
    csv_path = _write_orchestrator_fixture(tmp_path)
...
""",
    ...
    assert "Manual-only mode: reading market data from CSV cache" in (
...
def test_cli_rejects_external_provider_override(tmp_path: Path) -> None:
    csv_path = _write_orchestrator_fixture(tmp_path)
    config_path = tmp_path / "config.yaml"
...
""",
    ...
    try:
        main(["--config", str(config_path), "--provider", "yfinance", "--no-input"])
    except SystemExit as exc:
        assert exc.code == 2
    else:  # pragma: no cover - argparse should exit
        raise AssertionError("external provider override should be rejected")
